...
def create_sky(root, canvas):
    sky_1 = sky.Sky(root, canvas, config.initial_anchor_sky_1, config.initial_anchor_sky_x_1,
                    config.initial_anchor_sky_y_1, "Sky01")
    sky_2 = sky.Sky(root, canvas, tkinter.SW, sky_1.nw[0], sky_1.nw[1], "Sky02")

    tmp_canvas_img = canvas.create_image(sky_1.anchor_x, sky_1.anchor_y, anchor=sky_1.anchor,
                                         image=sky_1.bg_image, tags=sky_1.bg_image_tags)

    sky_1.set_canvas_image(tmp_canvas_img)

    tmp_canvas_img = canvas.create_image(sky_2.anchor_x, sky_2.anchor_y, anchor=sky_2.anchor,
                                         image=sky_2.bg_image, tags=sky_2.bg_image_tags)
...
def create_hero(root, canvas, lives):
    hero = heroplane.HeroPlane(root, canvas, config.initial_anchor_hero,
                               ...
                               "HeroPlane01", lives)
    tmp_canvas_img = canvas.create_image(hero.anchor_x, hero.anchor_y,
                                         anchor=hero.anchor,
                                         image=hero.bg_image,
...
def create_bullet_tags(root, canvas, anchor, x, y, tags):
    ...
    tmp_canvas_img = canvas.create_image(blt.anchor_x, blt.anchor_y,
                                         anchor=blt.anchor,
                                         image=blt.bg_image,
...
def create_bullet(root, canvas, mother, tag_id):
    # 每帧子弹
    blt_1 = create_bullet_tags(root, canvas, tkinter.CENTER,
                               mother.nw[0] + mother.width / 2,
                               mother.nw[1], "BulletMid" + str(tag_id))
    return blt_1
...
def start_game(event, root, canvas):
    ...
    while config.game_flag == 'start':
        ...
        # 碰撞测试
        enemy_list, own_list = check_collision(root, canvas, enemy_list, own_list)
        ...
        canvas.create_text((5, 5), fill='red', font=('Helvetica', 20, 'bold'),
                           text="得分：" + str(mygrade), anchor=tkinter.NW, tag="Grade")
        ...
        canvas.create_text((config.window_boundary_col / 2, 5), fill='blue',
                           ...
                           anchor=tkinter.N, tag="Highest")

        # 打印生命值
        canvas.delete("Lives")
        canvas.create_text((config.window_boundary_col - 10, 5),
                           fill='#000fff000', font=('Helvetica', 20, 'bold'),
                           text="生命：" + str(own_list[0].lives_num),
                           anchor=tkinter.NE, tag="Lives")
...
def main():
    ...
    # 创建画布
    window_canvas = tkinter.Canvas(root_window, width=config.window_boundary_col,
